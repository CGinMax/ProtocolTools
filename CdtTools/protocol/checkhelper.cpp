#include "checkhelper.h"

unsigned char CheckHelper::CRC8Array[256] = {
    
};

CheckHelper::CheckHelper()
{

}

unsigned short CheckHelper::CheckCRC16(unsigned char buff[], int offset, int len)
{
    unsigned short result = 0xffff;
    for (int i = 0; i < len; i++)
    {
        result ^= buff[i + offset];
        for (int j = 0; j < 8; j++)
        {
            bool flag = ((result & 0x0001) != 0);
            result >>= 1;
            if (flag) result ^= 0xa001;
        }
    }

    return result;
}

unsigned char CheckHelper::CheckCRC8(unsigned char buff[], int offset, int len)
{
    unsigned char result = 0;
    for (int i = 0; i < len; i++)
    {
        result ^= buff[i + offset];
        result = CheckHelper::CRC8Array[result];
    }

    result ^= 0xff;
    return result;
}

unsigned short CheckHelper::CheckSum16(unsigned char buff[], int offset, int len)
{
    unsigned short result = 0x0000;
    for (int i = 0; i < len; i++)
    {
        result += buff[i + offset];
    }

    return result;
}

unsigned char CheckHelper::CheckSum8(unsigned char buff[], int offset, int len)
{
    unsigned char result = 0x00;
    for (int i = 0; i < len; i++)
    {
        result += buff[i + offset];
    }

    return result;
}

// 获取字节个数
int CheckHelper::GetByteSize(eValType valType)
{
    if (valType == eValType::Int16 || valType == eValType::Uint16) return 2;
    if (valType == eValType::Int32 || valType == eValType::Uint32 || valType == eValType::Float32) return 4;
    if (valType == eValType::Double64 || valType == eValType::Int64 || valType == eValType::Uint64) return 8;
    return 1;
}
