...
void networkTest::test_crc16()
{
    ...
    std::list<uint8_t> completeMsg = {0xEB, 0x90, 0xEB, 0x90, 0xEB, 0x90, 0x20, 0x20, 1, 0, 2, 0, 2, 0x02, 0x00, 0x01, 0x01, 0x0a,0x0b};
...
{
    // header fine
    std::list<uint8_t> headerMsg = {0x90, 0x20, 0xEB, 0x90, 0xEB, 0x90, 0xEB, 0x90, 2, 0, 2, 0x02, 0x00, 0x01, 0x01, 0x0a,0x0b};
    auto headerRes = YBFrame::parseBytesToFrame(headerMsg);
    QVERIFY(headerRes.second != eYBParseResult::HeaderError);

    // header fine in tail
    headerMsg = {0x90, 0x20, 2, 0, 2, 0x02, 0xEB, 0x90, 0x01, 0x0a,0x0b, 0xEB, 0x90, 0xEB, 0x90, 0xEB, 0x90};
    headerRes = YBFrame::parseBytesToFrame(headerMsg);
    QVERIFY(headerRes.second != eYBParseResult::HeaderError);

    // header error
    headerMsg = {0x90, 0x20, 2, 0, 2, 0x02, 0xEB, 0x90, 0x90, 0xEB, 0x90, 0xEB, 0x90, 0x90, 0x01, 0x0a,0x0b};
    headerRes = YBFrame::parseBytesToFrame(headerMsg);
    QVERIFY(headerRes.second == eYBParseResult::HeaderError);
    headerMsg = {0x90, 0x20, 2, 0, 2, 0x02, 0xEB, 0x90, 0xEB, 0x90, 0xEB, 0x10, 0x20, 0x01, 0x0a,0x0b};
    headerRes = YBFrame::parseBytesToFrame(headerMsg);
    QVERIFY(headerRes.second == eYBParseResult::HeaderError);

    // not complete
    std::list<uint8_t> bodyMsg = {0xEB, 0x90, 0xEB, 0x90, 0xEB, 0x90, 0x20, 0x20, 1, 0, 2, 0, 2, /*0x02, 0x00, 0x01, 0x01, 0x0a, */0x0b};
    auto bodyMsgRes = YBFrame::parseBytesToFrame(bodyMsg);
    QVERIFY(bodyMsgRes.second == eYBParseResult::NotComplete);
    // no complete no crc
    bodyMsg = {0xEB, 0x90, 0xEB, 0x90, 0xEB, 0x90, 0x20, 0x20, 1, 0, 2, 0, 2, 0x02, 0x00, 0x01, 0x01/*, 0x0a,0x0b*/};
    bodyMsgRes = YBFrame::parseBytesToFrame(bodyMsg);
    QVERIFY(bodyMsgRes.second == eYBParseResult::NotComplete);
    // data length error
    bodyMsg = {0xEB, 0x90, 0xEB, 0x90, 0xEB, 0x90, 0x20, 0x20, 1, 0, 2, 0, 2, 0xaa, 0x00, 0x01, 0x01, 0x0a,0x0b};
    bodyMsgRes = YBFrame::parseBytesToFrame(bodyMsg);
    QVERIFY(bodyMsgRes.second == eYBParseResult::DataLengthError);

    // crc error
    std::list<uint8_t> crcMsg = {0xEB, 0x90, 0xEB, 0x90, 0xEB, 0x90, 0x20, 0x20, 1, 0, 2, 0, 2, 0x02, 0x00, 0x01, 0x01, 0x10,0x20};
    auto crcRes = YBFrame::parseBytesToFrame(crcMsg);
    QVERIFY(crcRes.second == eYBParseResult::CrcError);

    // complete
    std::list<uint8_t> completeMsg = {0xEB, 0x90, 0xEB, 0x90, 0xEB, 0x90, 0x20, 0x20, 1, 0, 2, 0, 2, 0x02, 0x00, 0x01, 0x01, 0x43,0x46};
    auto completeRes = YBFrame::parseBytesToFrame(completeMsg);
