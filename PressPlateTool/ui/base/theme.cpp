...
const QVector<QColor> Ui::Theme::palette ={
...
Ui::Theme * Ui::Theme::instance()
{
    static Theme theme;
    return &theme;
}

QColor Ui::Theme::rgba(int red, int green, int blue, qreal alpha)
{
...
QColor Ui::Theme::color(const QString &name)
{
    if (!m_colorSheet.contains(name)) {
        return {};
    }
    return m_colorSheet.value(name);
}

QColor Ui::Theme::color(Ui::Color color)
{
    if (checkOutofRange(color)) {
        return {};
    }
    return palette.at(static_cast<int>(color));
}

void Ui::Theme::setColor(const QString &name, const QColor &color)
{
    m_colorSheet.insert(name, color);
}

void Ui::Theme::setColor(const QString &name, Ui::Color color)
{
    if (checkOutofRange(color)) {
        return ;
    }
    setColor(name, palette.at(static_cast<int>(color)));
}

QColor Ui::Theme::defaultColor()
{
    return color(QLatin1String("default"));
}

QColor Ui::Theme::primaryColor()
{
    return color(QLatin1String("primary"));
}

QColor Ui::Theme::disabledColor()
{
    return color(QLatin1String("disabled"));
}

QColor Ui::Theme::textColor()
{
    return color(QLatin1String("text"));
}
...
void Ui::Theme::initColorPalette()
{
    ...
    setColor("accent3", Ui::Color::grey500);
    setColor("text", Ui::Color::darkBlack);
    setColor("alternateText", Ui::Color::white);
    setColor("canvas", Ui::Color::white);
    setColor("border", Ui::Color::grey300);
    setColor("disabled", Ui::Color::minBlack);
    setColor("disabled2", Ui::Color::faintBlack);
    setColor("disabled3", Ui::Color::grey300);
}

bool Ui::Theme::checkOutofRange(Ui::Color color)
{
    int index = static_cast<int>(color);
    return index >= palette.size() && index < 0;
}
